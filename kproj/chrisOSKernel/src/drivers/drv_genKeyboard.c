...
void kbd_handler_generic()
{
    ...
    case KEY_CTRL_DN:
        ...
        printd(DEBUG_KEYBOARD_DRIVER,"Ctrl down\n");
        goto timeToReturn;
    case KEY_CTRL_UP:
                cursorSavePosition();
                cursorMoveTo(74,0);
        ...
        printd(DEBUG_KEYBOARD_DRIVER,"Ctrl up\n");
        goto timeToReturn;
    case KEY_ALT_DN:
        kKeyStatus[INDEX_ALT]=1;
        printd(DEBUG_KEYBOARD_DRIVER,"Alt down\n");
        goto timeToReturn;
    case KEY_ALT_UP:
        kKeyStatus[INDEX_ALT]=0;
        printd(DEBUG_KEYBOARD_DRIVER,"Alt up\n");
        goto timeToReturn;
    case KEY_CAPSLOCK_UP:
        kKeyStatus[INDEX_CAPSLOCK]=!kKeyStatus[INDEX_CAPSLOCK];
        printd(DEBUG_KEYBOARD_DRIVER,"Capslock\n");
        goto timeToReturn;
...
//        case MAKE_DOWN: kKeyStatus[INDEX_DOWN]=0;break;
    ...
    if (rawKey==BREAK_RIGHT || rawKey==BREAK_LEFT || rawKey==BREAK_UP || rawKey==BREAK_DOWN)
        if (activeTTY->stdInWritePipe)
        {
            ...
            printd(DEBUG_KEYBOARD_DRIVER, "kbd_handler_generic: Raw key '%c' delivered to stdin pipe 0x%08X (CR3=0x%08X)\n",translatedKeypress, activeTTY->stdInWritePipe, CURRENT_CR3);
    ...
    if (!(rawKey & 0x80))
    {
        ...
        if (activeTTY->stdInWritePipe)
        {
            ...
            printd(DEBUG_KEYBOARD_DRIVER, "kbd driver: Translated key '%c' delivered to stdin pipe 0x%08X (CR3=0x%08X), ctrl status=0x%02x\n",translatedKeypress, activeTTY->stdInWritePipe, CURRENT_CR3, kKeyStatus[INDEX_CTRL]);
	    //NOTE: We are passing data but no process.  sigaction2 knows to not expect a process for SIGIO
            sys_sigaction2(SIGIO, NULL, (uintptr_t)activeTTY->stdInReadPipe, NULL);
        }
        else
