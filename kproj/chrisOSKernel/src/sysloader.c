...
bool putDataOnPages(uintptr_t CR3, uintptr_t virtAddr, void* file, bool writeFromFile, int size, byte nonFileWriteValue, _Bool mapOnly, elfInfo_t* elf)
{
    ...
    while(totalLeftToWrite>0)
    {
        ...
        if (writeFromFile)
        {
            ...
            fs_read(file, (void*)startPhysAddr,1, countToWrite);
    ...
    if (isPageMapped(CR3,startVirtAddr))    //Write overlaps previous one, write the whole thing without mapping and leave
    {
        ...
        if (countToWrite==0)
        {
            ...
            if (writeFromFile)
                ...
                fs_read(file, (void*)startPhysAddr, size, 1);
            else
...
bool elfLoadSections(void* file,elfInfo_t* elfInfo,uintptr_t CR3)
{
    ...
    fs_read(file, &elfInfo->hdr, sizeof(elfInfo->hdr), 1);
    ...
    fs_read(file,elfInfo->secHdrTable, elfInfo->hdr.e_shentsize, elfInfo->hdr.e_shnum);
    ...
    fs_read(file, elfInfo->pgmHdrTable, elfInfo->hdr.e_phentsize, elfInfo->hdr.e_phnum);
    ...
    if (!alreadyLoaded)
    {
        ...
        for (int cnt=0;cnt<elfInfo->secHdrRecordCount;cnt++)
        {
            if (elfInfo->secHdrTable[cnt].sh_type==SHT_STRTAB)
            {
                ...
                uint32_t lResult=fs_read(fPtr, (char*)elfInfo->dynamicInfo.strTableAddress[cnt],elfInfo->secHdrTable[cnt].sh_size,1);
        ...
        for (int cnt=0;cnt<elfInfo->secHdrRecordCount;cnt++)
        {
            ...
            else if (elfInfo->secHdrTable[cnt].sh_type==SHT_SYMTAB)
            {
                ...
                fs_read(fPtr,elfInfo->symTable,elfInfo->secHdrTable[cnt].sh_size,1);
            ...
            else if (elfInfo->secHdrTable[cnt].sh_type==SHT_RELA)
            {
                ...
                fs_read(fPtr, elfInfo->dynamicInfo.relATable,elfInfo->dynamicInfo.relATableSize,1);
            ...
            else if (elfInfo->secHdrTable[cnt].sh_type==SHT_REL)
            {
                ...
                fs_read(fPtr, elfInfo->dynamicInfo.relTable,elfInfo->secHdrTable[cnt].sh_size,1);
