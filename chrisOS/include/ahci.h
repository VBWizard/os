...
int check_type(HBA_PORT *port, uint32_t* sig);
void ahciProbePorts(HBA_MEM *abar);
void printAHCICaps();
bool ahciInit();
void port_rebase(HBA_PORT *port, int portno, uintptr_t remapBase);
void start_cmd(HBA_PORT *port);
void stop_cmd(volatile HBA_PORT *port);
void ahciIdentify(HBA_PORT* port, int deviceType);
int find_cmdslot(HBA_PORT *port);
void waitForPortIdle(HBA_PORT *port);
void ahciSetCurrentDisk(HBA_PORT* port);
int ahciRead(HBA_PORT* port, int sector, uint8_t* buffer, int sector_count);
...
//int write_ahci(HBA_PORT *port, uint32_t startl, uint32_t starth, uint32_tuint32_t count, WORD *buffer); 
HBA_MEM ABARs[10];
#endif	/* AHCI_H */
